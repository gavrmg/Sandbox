...
//import ru.orion.sandbox.Renderer.Vertex3texcoord;
...
public class Utils {
	public static FloatBuffer createFlippedBuffer(Vertex[] data) {
		...
		for(int i = 0; i < data.length;i++) {
			...
			buffer.put(data[i].getNormal().x);
			buffer.put(data[i].getNormal().y);
			buffer.put(data[i].getNormal().z);
		}
		buffer.flip();
		return buffer;
	}
...
//		data.get(buffer);
	...
	public static short[] runLengthCode(short[] array) {
