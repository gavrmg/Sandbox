...
public class Game {
	...
	public Game() {
		...
		light = new PointLight(0f,1f,-2f,1f,1f,1f,1f,1f,(float)0.7,(float)0.7);
		difflight = new DiffuseLight(0.5f,1f,1f,1f);
		...
		basicshader.setPointLight("testlight", light);
...
/*		for(int i = 0; i < meshes.length;i++) {
	...
	public void update() {
		...
		basicshader.updateUniforms(transform.getTransformation().mul(camera.getView()), camera.getProjection(), transform.getTransformation().mul(camera.getView()).invert().transpose(), color.set((float) Math.abs(Math.sin(temp))));
	}
